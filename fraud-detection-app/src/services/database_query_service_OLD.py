...
class DatabaseQueryService:
    ...
    def get_transactions_by_npi(self, npi: int, limit: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            cursor.execute("""
                SELECT transaction_id, npi, amount, transaction_date, 
    ...
    def count_transactions_by_status(self, status: Optional[str] = None) -> Dict[str, Any]:
        """Count transactions by status (held/released/paid) or all."""
        try:
            conn = sqlite3.connect(self.finance_db_path)
            cursor = conn.cursor()
            ...
            if status:
                ...
                cursor.execute("SELECT COUNT(*), COALESCE(SUM(amount), 0) FROM payment_transactions")
    ...
    def get_case_by_npi(self, npi: int) -> Dict[str, Any]:
        """Get investigation case details for a provider."""
        try:
            conn = sqlite3.connect(self.providers_db_path)
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            cursor.execute("""
                SELECT case_id, provider_id, investigation_date, 
                       investigator_report, analyst_report, supervisor_decision,
                       final_report, status
                FROM cases 
                WHERE provider_id = ?
            ...
            if not row:
                return {"success": False, "message": f"No investigation case found for NPI {npi}."}
    ...
    def get_open_cases(self, limit: int = 10) -> Dict[str, Any]:
        """Get all open investigation cases."""
        try:
            conn = sqlite3.connect(self.providers_db_path)
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            logger.error(f"Error getting open cases: {e}")
            return {"success": False, "message": f"Error: {str(e)}"}

    def count_cases_by_status(self) -> Dict[str, Any]:
        """Count investigation cases by status."""
        try:
            conn = sqlite3.connect(self.providers_db_path)
            cursor = conn.cursor()
    ...
    def get_system_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get case stats
            case_stats = self.count_cases_by_status()
            ...
            return {
                ...
                "investigations": {
                    ...
                    "closed_cases": case_stats.get("by_status", {}).get("closed", 0)
                }
            }
        except Exception as e:
